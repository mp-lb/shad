...
const CURVE_SAMPLES: readonly Point[] = Array.from(
  { length: 96 },
  (_, index) => {
...
export function DotmSquare19({
  ...
  const resolver = useMemo<DotAnimationResolver>(() => {
    return ({ isActive, row, col, phase }) => {
      ...
      const lead = Math.max(
        headInfluence(dot, headA),
        headInfluence(dot, headB),
      );

      const trail = Math.max(
        headInfluence(dot, trailA),
        headInfluence(dot, trailB),
      );

      const centerPulse =
        Math.exp(-(dot.x * dot.x + dot.y * dot.y) / 0.05) *
        (0.45 + 0.55 * lead);
